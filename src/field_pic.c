...
u8 LoadStaticFieldPic(const u32 *gfx, const u16 *pal, u8 x, u8 y, u16 spriteTagId)
{
    struct CompressedSpriteSheet sheet;
    struct SpritePalette palSheet;
    struct SpriteTemplate spriteTempl;
    struct OamData oam = {0};
    u8 sLastPicId = 0; // Used for deleting the sprite
    ...
    // Prepare the OAM
    oam.size = SPRITE_SIZE(64x64);
    oam.shape = SPRITE_SHAPE(64x64);
    oam.priority = 1;
